...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public ResponseEntity<ApiResponse> registerUser(RegisterDTO registerDTO) {
        if (userRepository.existsByEmailAndEnabledFalse(registerDTO.getEmail())) {
            ...
            user.setEmailCode(String.valueOf(new Random().nextInt(9000) + 1000));
    ...
    @Override
    public ResponseEntity<ApiResponse> forgetPassword(ForgetPasswordDTO passwordDTO) {
        User user = userRepository.findByEmail(passwordDTO.getEmail()).orElseThrow(() -> new NotFoundException("Bunday user topilmadi"));
        String emailCode = String.valueOf(new Random().nextInt(9000) + 1000);
        if (sendEmail(passwordDTO.getEmail(), emailCode)) {
            user.setEmailCode(emailCode);
            userRepository.save(user);
            return ResponseEntity.ok(ApiResponse.builder().message("Emailingizga tasdiqlash ko'di yuborildi").status(200).isSuccess(true).build());
        }
        throw new MainException("Kod yuborishda xatolik");
    }

    @Override
    public ResponseEntity<ApiResponse> verifyEmailNewPassword(String emailCode, ForgetNewPasswordDTO passwordDTO) {
        User user = userRepository.findByEmailAndEmailCode(passwordDTO.getEmail(), emailCode).orElseThrow(() -> new NotFoundException("Parol yoki email hato"));
        user.setEmailCode(null);
        user.setPassword(passwordDTO.getNewPassword());
        userRepository.save(user);
        return ResponseEntity.ok(ApiResponse.builder().message("Parolingiz muvafaqiyatli yangilandi").status(200).isSuccess(true).build());
    }
